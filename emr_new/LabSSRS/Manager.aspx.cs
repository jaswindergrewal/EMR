...
public partial class Manager : System.Web.UI.Page
{
	...
	private void ClearGroups()
	{
		...
		txtGroupHigh.Text = string.Empty;
		txtGroupLow.Text = string.Empty;
		txtNormHigh.Text = string.Empty;
		txtNormLow.Text = string.Empty;
		txtFemHigh.Text = string.Empty;
		txtFemLow.Text = string.Empty;
		txtFemNormHigh.Text = string.Empty;
		txtFemNormLow.Text = string.Empty;
		//txtGroupName.Text = reader["GroupName"].ToString();
		txtGroupTitle.Text = string.Empty;
		edDescrip.Content = string.Empty;
		edHigh.Content = string.Empty;
		edLow.Content = string.Empty;
		edNormal.Content = string.Empty;
		FemHigh.Content = string.Empty;
		FemLow.Content = string.Empty;
		FemNormal.Content = string.Empty;
		cboGraph.Checked = false;
	}
	...
	private string RemoveTrailingZeros(string val)
	{
		...
		int currPos = charVal.Length - 1;
		while (currPos > 0)
		{
			if (charVal[currPos] == '0')
			{
			...
			case "Edit":
				{
					...
					if (Session["GroupID"].ToString() != "0")
					{
						...
						txtSortOrder.Text = RemoveTrailingZeros(theGroup.SortOrder.ToString());
						txtGroupHigh.Text = RemoveTrailingZeros(theGroup.LongevityHighValue.ToString());
						txtGroupLow.Text = RemoveTrailingZeros(theGroup.LongevityLowValue.ToString());
						txtNormHigh.Text = RemoveTrailingZeros(theGroup.AcceptableHighValue.ToString());
						txtNormLow.Text = RemoveTrailingZeros(theGroup.AcceptableLowValue.ToString());
						txtFemHigh.Text = RemoveTrailingZeros(theGroup.FemaleLongevityHigh.ToString());
						txtFemLow.Text = RemoveTrailingZeros(theGroup.FemaleLongevityLow.ToString());
						txtFemNormHigh.Text = RemoveTrailingZeros(theGroup.FemaleAcceptableHigh.ToString());
						txtFemNormLow.Text = RemoveTrailingZeros(theGroup.FemaleAcceptableLow.ToString());
						//txtGroupName.Text = reader["GroupName"].ToString();
						...
						FemHigh.Content = theGroup.HighFemale;
						FemLow.Content = theGroup.LowFemale;
						FemNormal.Content = theGroup.NormalFemale;
						edNormal.Content = Server.HtmlEncode(theGroup.NormalText);
						cboGraph.Checked = theGroup.ShowGraph;
						txtChartBottom.Text = RemoveTrailingZeros(theGroup.ChartBottom.ToString());
					}
					else
					{
						ClearGroups();
			...
			case "Edit":
				{
					...
					txtPanelSort.Text = thePanel.SortOrder.ToString();
	...
	protected void btnOkPanelInfo_Click(object sender, EventArgs e)
	{
		...
		if ((string)Session["PanelID"] == "0")
		{
			...
			pnl.SortOrder = int.Parse(txtPanelSort.Text);
		...
		else
		{
			...
			thePanel.SortOrder = int.Parse(txtPanelSort.Text);
			ctx.SubmitChanges();
	...
	protected void btnOkGroupInfo_Click(object sender, EventArgs e)
	{
		...
		if (Session["GroupID"].ToString() == "0")
		{
			...
			theGroup.SortOrder = int.Parse(txtSortOrder.Text);
			...
			theGroup.AcceptableHighValue = decimal.Parse(txtNormHigh.Text);
			theGroup.AcceptableLowValue = decimal.Parse(txtNormLow.Text);
			theGroup.FemaleLongevityHigh = decimal.Parse(txtFemHigh.Text);
			theGroup.FemaleLongevityLow = decimal.Parse(txtFemLow.Text);
			theGroup.FemaleAcceptableHigh = decimal.Parse(txtFemNormHigh.Text);
			theGroup.FemaleAcceptableLow = decimal.Parse(txtFemNormLow.Text);
			theGroup.HighFemale = FemHigh.Content;
			theGroup.NormalFemale = FemNormal.Content;
			theGroup.LowFemale = FemLow.Content;
			theGroup.ShowGraph = cboGraph.Checked;
			theGroup.ChartBottom = decimal.Parse(txtChartBottom.Text);
			ctx.LabReports_Groups.InsertOnSubmit(theGroup);
			ctx.SubmitChanges();
		}
		else
		{
			LabReports_Group theGroup = (from g in ctx.LabReports_Groups where g.GroupID == int.Parse(Session["GroupID"].ToString()) select g).First();
	...
	protected void mnuTriggerGroupn_Command(Object sender, FlyContextMenuCommandEventArgs e)
	{
		FlyTreeNode theNode = Panels.FindByID(e.CommandArgument);
		switch (e.CommandName)
		{
			case "Remove":
				{
					List<LabReports_ConditionDetail> dets = (from d in ctx.LabReports_ConditionDetails
															 join c in ctx.LabReports_Conditions on d.ConditionID equals c.ConditionID
															 where d.GroupID == int.Parse(theNode.Value)
															 && c.TriggerID == int.Parse(theNode.Parent.Value)
															 select d).ToList();

					if (dets.Count == 0)
					{
						LabReports_TriggersGroup trig = (from tg in ctx.LabReports_TriggersGroups
														 where tg.TriggerGroupID == int.Parse(theNode.Value)
														 select tg).First();

						ctx.LabReports_TriggersGroups.DeleteOnSubmit(trig);
						ctx.SubmitChanges();
					}
					break;
				}
		...
		foreach (var panel in dbPanels)
		{
			...
			foreach (var theGroup in dbGroups)
			{
				...
				foreach (var test in dbTests)
				{
					...
					theTest.ToolTip = "Last Date: " + (test.LastUsed).ToShortDateString();
			...
			foreach (var trigger in dbTriggers)
			{
				...
				foreach (var group in triggerGroups)
				{
					...
					GroupNode.ContextMenuID = "mnuTriggerGroup";
					GroupNode.DragDropAcceptNames = "";
					GroupNode.Value = group.TriggerGroupID.ToString();
