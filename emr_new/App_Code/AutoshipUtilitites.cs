...
public class AutoshipUtilities
{
    ...
    public DataTable PopulateTree(FlyTreeView theTree, int PatientID)
    {
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString))
        {
            ...
            System.Web.HttpContext.Current.Session["QuitAutoship"] = "false";
            foreach (DataRow dr in dt.Rows)
            {
                ...
                else
                {
                    ...
                    System.Web.HttpContext.Current.Session["QuitAutoship"] = "true";
    ...
    public void PopulateProducts(FlyTreeView tree, int patientId)
    {
        ...
        while (true)
        {
            ...
            bool Issuppliment = false;
            foreach (var item in lstSupplierDetails)
            {
                if (item.ProductID == (int)reader["ProductId"])
                {
                    Issuppliment = true;
            ...
            FlyTreeNode newNode;
            if (Issuppliment == true)
                newNode = new FlyTreeNode("<font color = 'red'>" + (string)reader["ProductName"] + "</font>");

            // newNode.Text = " < font color = 'red' > " + newNode.Text + "</font>";
            else
                newNode = new FlyTreeNode((string)reader["ProductName"]);
    ...
    public void PopulateProblemProducts(FlyTreeView tree)
    {
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString))
        {
            ...
            con.Open();
            using (SqlCommand cmd = new SqlCommand("ssp_GetSuppliments", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;

                SqlDataReader rd = Utils.OpenReader(cmd);//cmd1.ExecuteReader();
                FlyTreeNode node = new NineRays.WebControls.FlyTreeNode("Suppliment List");

                tree.Nodes.Add(node);
                tree.Nodes[0].ImageUrl = "$vista_folder";
                FlyTreeNode GroupNode = new FlyTreeNode();
                FlyTreeNode SupplimentNode = new FlyTreeNode();
                int GroupId = 0;
                while (rd.Read())
                {
                    if (GroupId == (int)rd["GroupId"] && (int)rd["GroupId"] > 0)
                    {
                        SupplimentNode = new FlyTreeNode((string)rd["ProductName"]);
                        SupplimentNode.CanBeSelected = true;
                        SupplimentNode.DragDropAcceptNames = "";
                        SupplimentNode.DragDropName = "Product";
                        SupplimentNode.Value = rd["ProductID"].ToString();
                        GroupNode.ChildNodes.Add(SupplimentNode);
                    }
                    else if (GroupId != (int)rd["GroupId"] && (int)rd["GroupId"] > 0)
                    {
                        string GroupName = (string)rd["GroupName"];

                        string ProductName = (string)rd["ProductName"];
                        GroupId = (int)rd["GroupId"];
                        GroupNode = new FlyTreeNode(GroupName);
                        GroupNode.DragDropName = "";
                        GroupNode.CanBeSelected = false;
                        GroupNode.Expanded = false;
                        GroupNode.ImageUrl = "$vista_folder";
                        GroupNode.Value = GroupId.ToString();
                        tree.Nodes[0].ChildNodes.Add(GroupNode);
                    ...
                    else
                    {
                        ...
                        SupplimentNode.DragDropName = "Product";
                        SupplimentNode.Value = rd["ProductID"].ToString();
                        tree.Nodes[0].ChildNodes.Add(SupplimentNode);
                    }
                }
    ...
    public void PopulateProblemList(FlyTreeView tree, int patientId)
    {
        //    SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString);
        //    conn.Open();
        //    SqlCommand cmd = new SqlCommand();
        //    cmd.CommandText = "Select p.* from Icd10Codes p where p.IsActive = 1  order by ICD10Code";
        //    cmd.Connection = conn;
        //    cmd.CommandType = CommandType.Text;
        //    SqlDataReader reader = OpenReader(cmd);
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString))
        {
            ...
            con.Open();
            using (SqlCommand cmd = new SqlCommand("ssp_GetICDCodesWithSuppliments", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;

                SqlDataReader rd = Utils.OpenReader(cmd);//cmd1.ExecuteReader();
                FlyTreeNode node = new NineRays.WebControls.FlyTreeNode("Problem List");

                tree.Nodes.Add(node);
                tree.Nodes[0].ImageUrl = "$vista_folder";
                FlyTreeNode ProblemNode = new FlyTreeNode();
                FlyTreeNode SupplimentNode = new FlyTreeNode();
                int Id = 0;
                while (rd.Read())
                {
                    if (Id == (int)rd["Id"])
                    {
                        ...
                        SupplimentNode.DragDropName = "Product";
                        SupplimentNode.Value = rd["ProductID"].ToString();
                        ProblemNode.ChildNodes.Add(SupplimentNode);
                    }
                    else
                    {
                        string IcdCode = (string)rd["ICD10Code"];
                        string Description = (string)rd["Description"];
                        string Gender = (string)rd["Gender"];
                        string ProductName = (string)rd["ProductName"];
                        Id = (int)rd["Id"];
                        ProblemNode = new FlyTreeNode(IcdCode + " - " + Description + " (" + Gender + ")");
                        ProblemNode.DragDropName = "";
                        ProblemNode.CanBeSelected = false;
                        ProblemNode.Expanded = false;
                        ProblemNode.ImageUrl = "$vista_folder";
                        ProblemNode.Value = rd["Id"].ToString();
                        tree.Nodes[0].ChildNodes.Add(ProblemNode);
    ...
    public void PopulateSuppliments(FlyTreeView tree, int patientId)
    {
        IPrescriptionService objPrescriptionService = null;
        List<PrescriptionSupplierViewModel> lstSupplierDetails = null;
        ...
        FlyTreeNode node = new NineRays.WebControls.FlyTreeNode("Suppliments ");
        ...
        //int thisId = ((int)reader["GroupID"]) + 1;
        FlyTreeNode thisNode = new FlyTreeNode(); ;//= null; //Tester.Nodes[0];
        foreach (var i in lstSupplierDetails)
        {


            thisNode = new FlyTreeNode(i.SuppName);
            thisNode.DragDropName = "";
            thisNode.CanBeSelected = false;
            thisNode.Expanded = false;
            //thisNode.ImageUrl = "$vista_folder";
    ...
    public void SaveProblemItems(FlyTreeView sked)
    {
            ...
            foreach (FlyTreeNode Group in sked.Nodes[0].ChildNodes)
            {
                ...
                foreach (FlyTreeNode Item in Group.ChildNodes)
                {
                    ...
                    try
                    {
                        ...
                        ItemID = int.Parse(Item.Value);
                        ICDCodeId = int.Parse(Group.Value);
                    AutoshipProductDAL obj = new AutoshipProductDAL();
                    obj.InsertProplemSuppliments(ICDCodeId, ItemID);
    ...
    public void AssembleContactRecs(FlyTreeView Sked, FlyTreeView Future, List<ScheduleGroup> ChangedGroups, List<ScheduleGroup> originalGroups, DataTable originalStanding, int PatientID, int sID)
    {
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["db"].ConnectionString))
        {
            ...
            while (reader.Read())
            {
                ...
                MessageBody += CheckHold(Sked, (int)reader["ProfileItemID"], (int)reader["Hold"]);
                ChangesMade = true;
    ...
    private string CheckHold(FlyTreeView theTree, int ProfileItemID, int Hold)
    {
        FlyTreeNode node = theTree.FindByValue(ProfileItemID.ToString());
        if (node != null)
        {
            if (node.Parent.ContextMenuID == "GroupMenuNew" && Hold == 0)
            {
                return node.Text + " Product Group is changed to hold .<br/>";
            }
            else if (node.Parent.ContextMenuID == "GroupMenu" && Hold == 1)
            {
                return node.Text + " Product Group is changed to unhold .<br/>";
            }
            else
                return "";
        }
    ...
    private string AddSkedItem(FlyTreeNode theNode, string MessageBody, DataTable originalStanding)
    {
        ...
        if (!MessageBody.Contains("The following standing schedule changes were made."))
        {
            ...
            MessageBody += theNode.Text + " " + theNode.Parent.Text + "<br/>";
        }
        /*string ProdName = ExtractProdName(theNode.Text);
        DataRow theOne = null;
        foreach (DataRow dr in originalStanding.Rows)
        {
            if (string.Equals((string)dr["ProductName"],ProdName))
            {
                theOne = dr;
                break;
            }
        ...
        if (theOne != null)
        {
            ...
            if (!MessageBody.Contains("Previous Entry: "))
                MessageBody +=  Prompt+ " " + theNode.Parent.Text + "<br/>";
            else
              MessageBody += "Previous Entry: " + Prompt + " " + theNode.Parent.Text + "<br/>";
        }
        else*/
        else
            MessageBody = "New Entry: " + theNode.Text + " " + theNode.Parent.Text + "<br/>";
    ...
    public bool SendEmail(int PatientID)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select p.Lastname,p.FirstName,p.PatientID,p.Email,e.TemplateDesc from patients p ,autoshipemailtemplate e where patientID=" + PatientID.ToString(), conn);
            cmd.CommandType = CommandType.Text;
            ...
            if ((string)dt.Rows[0]["Email"] != "")
            {
                ...
                string MessageBody = (string)dt.Rows[0]["TemplateDesc"];
                MessageBody = MessageBody.Replace("{UserName}", (string)dt.Rows[0]["FirstName"] + " " + (string)dt.Rows[0]["Lastname"]);
                ...
                string OrderData = AddOrderData((string)appReader.GetValue("shipmentLineSingle", docFooter.GetType()),
                ...
                MessageBody = MessageBody.Replace("{OrderData}", OrderData);
                SendMessage("ContactUs@longevitymedicalclinic.com", "Longevity Medical Clinic", (string)dt.Rows[0]["Email"], (string)dt.Rows[0]["FirstName"] + " " + (string)dt.Rows[0]["Lastname"], MessageBody, "", "Your supplements order");
                return true;
            }
            else
            {
    ...
    public static void SendFileShareMessage(int PatientID,string FileName,string oringinalFileName)
    {
        ...
        {
            conn.Open();
            SqlCommand cmd = new SqlCommand("select p.Lastname,p.FirstName,p.PatientID,p.Email,(Select top 1 e.TemplateDesc from emailTemplate e where appointmentid=0) As TemplateDesc from patients p where patientID=" + PatientID.ToString(), conn);
            cmd.CommandType = CommandType.Text;

            DataTable dt = OpenTable(cmd);
            string URL="<a href='https://lmc-sharefiles.s3-us-west-2.amazonaws.com/"+PatientID.ToString()+"/"+FileName+ "' download='https://lmc-sharefiles.s3-us-west-2.amazonaws.com/" + PatientID.ToString() + "/" + FileName + "'</a>" + oringinalFileName;//
            if ((string)dt.Rows[0]["Email"] != "")
            {
                //string docFooter = "";
                //Build message body
                ...
                SendMessage("ContactUs@longevitymedicalclinic.com", "Longevity Medical Clinic", (string)dt.Rows[0]["Email"], (string)dt.Rows[0]["FirstName"] + " " + (string)dt.Rows[0]["Lastname"], MessageBody, "", "New File Shared");
    ...
    public static void SendMessage(string fromAddress, string fromName, string toAddress, string toName, string MessageBody, string Attachment, string Subject)
    {
        var smtp = new SmtpClient
        {
            ...
            Credentials = new System.Net.NetworkCredential("contactus@Longevitymedicalclinic.com", "StarFish44!")
        };

        using (var message = new MailMessage("contactus@Longevitymedicalclinic.com", toAddress)
        {
            Subject = Subject,
            Body = MessageBody,
            IsBodyHtml = true,
